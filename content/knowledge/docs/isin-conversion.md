...
└── reconcile(offering_id, clearstream_snapshot_hash)
    ...
    → On failure: freezes CrossConversion for the affected offering</code></pre>
        ...
        <ol>
            <li><strong>Pull Clearstream positions</strong> - The Reconciliation Engine requests the daily position report from Clearstream for all active ISINs linked to CCASH offerings.</li>
